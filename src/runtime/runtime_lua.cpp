...
int Runtime::lua_vector(lua_State* state) {
    ...
    lua_pushnumber(state, x); lua_setfield(state, -2, "x");
    lua_pushnumber(state, y); lua_setfield(state, -2, "y");
    lua_pushnumber(state, z); lua_setfield(state, -2, "z");
    lua_pushliteral(state, "Vector"); lua_setfield(state, -2, "__type");
    luaL_getmetatable(state, kValueMeta); lua_setmetatable(state, -2);
    return 1;
}
...
int Runtime::lua_color(lua_State* state) {
    ...
    auto valid = [](lua_Integer value) { return value >= 0 && value <= 255; };
    if (!valid(r) || !valid(g) || !valid(b) || !valid(a)) return luaL_error(state, "Color channels must be between 0 and 255");
    lua_createtable(state, 0, 5);
    lua_pushinteger(state, r); lua_setfield(state, -2, "r");
    lua_pushinteger(state, g); lua_setfield(state, -2, "g");
    lua_pushinteger(state, b); lua_setfield(state, -2, "b");
    lua_pushinteger(state, a); lua_setfield(state, -2, "a");
    lua_pushliteral(state, "Color"); lua_setfield(state, -2, "__type");
    luaL_getmetatable(state, kValueMeta); lua_setmetatable(state, -2);
    return 1;
}
...
int Runtime::lua_value_tostring(lua_State* state) {
    ...
    if (type && std::string_view(type) == "Vector") {
        lua_getfield(state, 1, "x"); const double x = lua_tonumber(state, -1); lua_pop(state, 1);
        lua_getfield(state, 1, "y"); const double y = lua_tonumber(state, -1); lua_pop(state, 1);
        lua_getfield(state, 1, "z"); const double z = lua_tonumber(state, -1); lua_pop(state, 1);
        result << "Vector(" << x << ", " << y << ", " << z << ")";
    } else {
        lua_getfield(state, 1, "r"); const auto r = lua_tointeger(state, -1); lua_pop(state, 1);
        lua_getfield(state, 1, "g"); const auto g = lua_tointeger(state, -1); lua_pop(state, 1);
        lua_getfield(state, 1, "b"); const auto b = lua_tointeger(state, -1); lua_pop(state, 1);
        lua_getfield(state, 1, "a"); const auto a = lua_tointeger(state, -1); lua_pop(state, 1);
...
int Runtime::lua_module_searcher(lua_State* state) {
    ...
    if (path.empty() || !std::filesystem::exists(path)) {
        lua_pushfstring(state, "\n\tno LuaCS module '%s' in %s", name, runtime->bin_dir_.string().c_str());
        return 1;
    }
...
int Runtime::lua_module_loader(lua_State* state) {
    ...
    auto* vm = static_cast<ScriptVm*>(lua_touserdata(state, -1));
    lua_pop(state, 1);
    if (!runtime || !vm) return luaL_error(state, "LuaCS runtime state is unavailable");
    ...
    if (!handle) return luaL_error(state, "could not load native module %s (Windows error %lu)", path, GetLastError());
    auto open = reinterpret_cast<OpenModuleFn>(GetProcAddress(handle, "LuaCS_OpenModule"));
    if (!open) {
        FreeLibrary(handle);
    ...
    if (!lua_istable(state, -1)) {
        lua_pop(state, 1);
        FreeLibrary(handle);
        return luaL_error(state, "module %s returned an invalid module table", module_name);
    }
...
int Runtime::lua_native_open_trampoline(lua_State* state) {
    auto open = reinterpret_cast<OpenModuleFn>(lua_touserdata(state, lua_upvalueindex(1)));
    auto* services = static_cast<const Services*>(lua_touserdata(state, lua_upvalueindex(2)));
    if (!open || !services) return luaL_error(state, "native module loader state is invalid");
    const int results = open(state, services);
    if (results != 1) return luaL_error(state, "native module must return exactly one value");
    return 1;
}
...
int Runtime::lua_traceback(lua_State* state) {
    ...
    if (message) luaL_traceback(state, state, message, 1); else lua_pushliteral(state, "(non-string Lua error)");
    return 1;
}

bool Runtime::install_core(ScriptVm& vm) {
    lua_State* state = vm.state;
    lua_pushlightuserdata(state, this); lua_setfield(state, LUA_REGISTRYINDEX, kRuntimeRegistryKey);
    lua_pushlightuserdata(state, &vm); lua_setfield(state, LUA_REGISTRYINDEX, kVmRegistryKey);

    lua_pushcfunction(state, &Runtime::lua_print); lua_setglobal(state, "print");
    lua_pushcfunction(state, &Runtime::lua_vector); lua_setglobal(state, "Vector");
    lua_pushcfunction(state, &Runtime::lua_color); lua_setglobal(state, "Color");

    if (luaL_newmetatable(state, kValueMeta)) {
        lua_pushcfunction(state, &Runtime::lua_value_tostring); lua_setfield(state, -2, "__tostring");
    }
    lua_pop(state, 1);

    lua_getglobal(state, "package");
    lua_getfield(state, -1, "searchers");
    const lua_Integer length = static_cast<lua_Integer>(lua_rawlen(state, -1));
    ...
    const auto require_global = [&](const char* module_name, const char* global_name) {
        lua_getglobal(state, "require");
        lua_pushstring(state, module_name);
        if (!protected_call(vm, 1, 1, std::string("loading core module '") + module_name + "'")) {
            return false;
        }
...
bool Runtime::protected_call(ScriptVm& vm, int argument_count, int result_count,
    ...
    const int status = lua_pcall(vm.state, argument_count, result_count, function_index);
    lua_remove(vm.state, function_index);
    if (status == LUA_OK) return true;
    const char* message = lua_tostring(vm.state, -1);
    log(vm, std::string(context) + " failed: " + (message ? message : "unknown Lua error"));
    lua_pop(vm.state, 1);
    return false;
}

void Runtime::push_player(lua_State* state, const PlayerSnapshot& player) const {
    ...
    push_string_field(state, "steamid", player.steam_id.empty() ? steam2_from_steam64(player.steam64) : player.steam_id);
...
std::pair<std::string, std::string> Runtime::parse_command(std::string_view command_line) {
    ...
    std::string rest = first_space == std::string::npos ? std::string{} : trim(std::string_view(raw).substr(first_space + 1));
    if (first == "say" || first == "say_team") {
        if (rest.size() >= 2 && rest.front() == '"' && rest.back() == '"') rest = rest.substr(1, rest.size() - 2);
        if (rest.empty() || (rest.front() != '!' && rest.front() != '/')) return {};
        rest.erase(rest.begin());
        const auto split = rest.find_first_of(" \t");
        return {normalize_name(rest.substr(0, split)), split == std::string::npos ? std::string{} : trim(std::string_view(rest).substr(split + 1))};
    }
    return {first, rest};
}
