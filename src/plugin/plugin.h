...
public:
    ...
    void OnLevelInit(const char* map_name, const char* map_entities, const char* old_level,
                     const char* landmark_name, bool load_game, bool background);
    void OnLevelShutdown();

    void Hook_GameFrame(bool simulating, bool first_tick, bool last_tick);
    void Hook_ClientActive(CPlayerSlot slot, bool load_game, const char* name, uint64 xuid);
    ...
    void Hook_OnClientConnected(CPlayerSlot slot, const char* name, uint64 xuid,
                                const char* network_id, const char* address, bool fake_player);
    void Hook_ClientCommand(CPlayerSlot slot, const CCommand& command);

    const char* GetAuthor() override { return "Memphis-Cat / LuaCS contributors"; }
